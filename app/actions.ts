...
export async function createWeeklyEntry(input: unknown) {
  ...
  return prisma.weeklyEntry.upsert({
    ...
    update: {
      weekEnd: end,
      earnings: new Prisma.Decimal(parsed.earnings),
      trips: parsed.trips,
...
export async function updateWeeklyEntry(id: number | string, input: Partial<WeeklyEntryInput>) {
  ...
  const patch: Prisma.WeeklyEntryUpdateInput = {};
  ...
  if (typeof parsed.earnings === "number") patch.earnings = new Prisma.Decimal(parsed.earnings);
